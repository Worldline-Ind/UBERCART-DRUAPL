...
class Worldline extends PaymentMethodPluginBase  implements OffsitePaymentMethodPluginInterface
{
  ...
  public function defaultConfiguration()
  {
    return [
      ...
      'enableExpressPay' => 1,
      'separateCardMode' => 0,
      'worldline_success_msg' => 'Thank you for shopping with us. Your account has been charged and your transaction is successful.',
      'worldline_decline_msg' => 'Thank you for shopping with us. However, the transaction has been declined.',
      ...
      'disclaimerMsg' => 'Customize disclaimer message from merchant which will be shown to customer in checkout page',
      'merchantMsg' => 'Customize message from merchant which will be shown to customer in checkout page',
      'separateCardMode' => 0,
      'enableMerTxnDetails' => 1,
      'enableInstrumentDeRegistration' => 0,
      'hideSavedInstruments' => 0,
      'saveInstrument' => 0,
      'txnType' => 'SALE',
      'handle_response_on_popup' => 'no',
      'checkoutElement' => '',
    ];
  }
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state)
  {
    ...
    $form['worldline_success_msg'] = array(
      ...
      '#default_value' => $this->configuration['worldline_success_msg'],
    );
    $form['worldline_decline_msg'] = array(
      '#type' => 'textarea',
      '#title' => $this->t('Decline Message'),
      '#description' => $this->t('Decline Message'),
      '#default_value' => $this->configuration['worldline_decline_msg'],
    );
    $form['merchant_logo_url'] = array(
      ...
      '#default_value' => $this->configuration['merchant_logo_url'],
    ...
    $form['PRIMARY_COLOR_CODE'] = array(
      ...
      '#default_value' => $this->configuration['PRIMARY_COLOR_CODE'],
    );
    $form['SECONDARY_COLOR_CODE'] = array(
      '#type' => 'textfield',
      '#title' => $this->t('SECONDARY Color Code'),
      '#description' => $this->t('Color value can be hex, rgb or actual color name'),
      '#default_value' => $this->configuration['SECONDARY_COLOR_CODE'],
    );
    $form['BUTTON_COLOR_CODE_1'] = array(
      '#type' => 'textfield',
      '#title' => $this->t('Button Color Code 1'),
      '#description' => $this->t('Color value can be hex, rgb or actual color name'),
      '#default_value' => $this->configuration['BUTTON_COLOR_CODE_1'],
    );
    $form['BUTTON_COLOR_CODE_2'] = array(
      '#type' => 'textfield',
      '#title' => $this->t('Button Color Code 2'),
      '#description' => $this->t('Color value can be hex, rgb or actual color name'),
      '#default_value' => $this->configuration['BUTTON_COLOR_CODE_2'],
    );
    ...
    $form['enableExpressPay'] = array(
      ...
      '#default_value' => $this->configuration['enableExpressPay'],
    );
    $form['separateCardMode'] = array(
      ...
      '#default_value' => $this->configuration['separateCardMode'],
    );
    $form['merchantMsg'] = array(
      '#type' => 'textfield',
      '#title' => $this->t('Merchant Message'),
      '#description' => $this->configuration['merchantMsg'],
    );
    $form['disclaimerMsg'] = array(
      '#type' => 'textfield',
      '#title' => $this->t('Disclaimer Message'),
      '#description' => $this->configuration['disclaimerMsg'],
    );
    $form['enableMerTxnDetails'] = array(
      ...
      '#default_value' => $this->configuration['enableMerTxnDetails'],
    );
    $form['enableInstrumentDeRegistration'] = array(
      ...
      '#default_value' => $this->configuration['enableInstrumentDeRegistration'],
    );
    $form['hideSavedInstruments'] = array(
      ...
      '#default_value' =>  $this->configuration['hideSavedInstruments'],
    );
    $form['saveInstrument'] = array(
      ...
      '#default_value' => $this->configuration['saveInstrument'],
    );
    $form['txnType'] = array(
      '#type' => 'select',
      ...
      '#default_value' => $this->configuration['txnType'],
    );
    ...
    $form['handle_response_on_popup'] = array(
      ...
      '#default_value' => $this->configuration['handle_response_on_popup'],
    );
  ...
  public function buildRedirectForm(array $form, FormStateInterface $form_state, OrderInterface $order = NULL)
  {
    ...
    $config->set('enableExpressPay', $this->configuration['enableExpressPay']);
    $config->set('separateCardMode', $this->configuration['separateCardMode']);
    $config->set('worldline_success_msg', $this->configuration['worldline_success_msg']);
    $config->set('merchant_logo_url', $this->configuration['merchant_logo_url']);
    $config->set('PRIMARY_COLOR_CODE', $this->configuration['PRIMARY_COLOR_CODE']);
    $config->set('BUTTON_COLOR_CODE_1', $this->configuration['BUTTON_COLOR_CODE_1']);
    $config->set('BUTTON_COLOR_CODE_2', $this->configuration['BUTTON_COLOR_CODE_2']);
    $config->set('worldline_payment_mode', $this->configuration['worldline_payment_mode']);
    $config->set('SECONDARY_COLOR_CODE', $this->configuration['SECONDARY_COLOR_CODE']);
    $config->set('separateCardMode', $this->configuration['separateCardMode']);
    $config->set('merchantMsg', $this->configuration['merchantMsg']);
    $config->set('disclaimerMsg', $this->configuration['disclaimerMsg']);
    $config->set('enableMerTxnDetails', $this->configuration['enableMerTxnDetails']);
    $config->set('enableInstrumentDeRegistration', $this->configuration['enableInstrumentDeRegistration']);
    $config->set('hideSavedInstruments', $this->configuration['hideSavedInstruments']);
    $config->set('saveInstrument', $this->configuration['saveInstrument']);
    $config->set('txnType', $this->configuration['txnType']);
    $config->set('handle_response_on_popup', $this->configuration['handle_response_on_popup']);
    $config->save(TRUE);
